...
public class Main_3 {
    ...
    public static int compareVersion(String version1, String version2) throws Exception {
     ...
     /*   String[] versionArray1 = version1.split("\\.");//注意此处为正则匹配，不能用"."；
        ...
        String[] versionArray1 = new String[20];
        String[] versionArray2 = new String[20];
        somePreHandle(versionArray1, version1);
        somePreHandle(versionArray2, version2);
    ...
    public static void somePreHandle(String[] arr, String version) {
        arr = version.split("\\.");//注意此处为正则匹配，不能用"."；
        for (int i = 0; i < arr.length; i++) { //如果位数只有一位则自动补零（防止出现一个是04，一个是5 直接以长度比较）
            if (arr[i].length() == 1) {
                arr[i] = "0" + arr[i];
            }
        }
